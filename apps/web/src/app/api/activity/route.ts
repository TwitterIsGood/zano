...
import {
  LIVE_AGENT_EVENT_TYPES,
  resolveLatestAgentActivities,
  type LiveAgentEventRow,
  type RuntimeSessionRow,
} from "./latest-agent-activities";

async function loadLatestAgentActivities(admin: ReturnType<typeof createAdminClient>, serverId: string) {
  const [activityResult, sessionsResult] = await Promise.all([
    admin
      .from("member_activity_events")
      .select("actor_id,event_type,label,summary,occurred_at,created_at,channel_id,message_id,thread_parent_id,task_id")
      .eq("server_id", serverId)
      .eq("actor_type", "agent")
      .in("visibility", ["server", "public"])
      .in("event_type", LIVE_AGENT_EVENT_TYPES)
      .order("occurred_at", { ascending: false })
      .limit(500),
    admin
      .from("daemon_runtime_sessions")
      .select("agent_id,state,started_at,last_active_at,idle_at,ended_at,last_error")
      .eq("workspace_id", serverId)
  ...
  return {
    activities: resolveLatestAgentActivities(
      (activityResult.data ?? []) as LiveAgentEventRow[],
      (sessionsResult.data ?? []) as RuntimeSessionRow[]
    ),
  };
}
...
export async function GET(request: NextRequest) {
  ...
  if (latest === "agents") {
    const result = await loadLatestAgentActivities(admin, serverId);
