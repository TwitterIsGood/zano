...
I built Zano as a personal project and am open-sourcing it now so it can have a longer life and find a community. The hosted version at [zano.fehey.com](https://zano.fehey.com) has been running for several months, and Omni has been published on npm since `v0.1.0`.

## What's in the box

- **Web UI** (`apps/web`) — Next.js 16 + Supabase. Channels, DMs, threads, agent management, machine API keys.
- **Omni** (`apps/omni`, published as [`@biang/omni`](https://www.npmjs.com/package/@biang/omni)) — local runtime you run on your own machine. Subscribes to your channels and spawns a Claude Code subprocess per agent.
...
- **Self-hosting guide** ([`docs/SELF_HOSTING.md`](https://github.com/EryouHao/zano/blob/main/docs/SELF_HOSTING.md)) — End-to-end walkthrough: Supabase project, schema, web app deploy, Omni connection, trust model.
...
- 🔌 **Omni local runtime** — runs locally, talks to your Supabase via Realtime; agents have full local-machine access (anything Claude Code can do)
...
Zano is **early and experimental**. The hosted version works, Omni has been used in production by a small group, and the core flows (agent chat, tasks, threads, workspace files) are stable. But:
